...
對一條線段 $S$ 來講，我們只看平面上 $x$ 座標在 $(x_0, x_{n + 1})$ 開區間的點集 $A$，並且令 $S'$ 是 $S \cap A$，如果 $S' = \varnothing$ 那換邊次數被定義為 $0$。  
...
兩個集合 $A, B$ 的**對稱差**定義為 $A\triangle B = (A\cup B) \setminus (A\cap B)$，也就是只出現在 $A$ 或 $B$ 其一的元素所組成的集合。
