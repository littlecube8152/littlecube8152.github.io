...
使得每個分別的子序列中最常出現的數都不超過 $\Big\lceil \frac{\ell}{2} \Big\rceil$，其中 $\ell$ 是子序列的長度。
...
設區間長度為 $\ell$，區間眾數出現 $c$ 次，
則有一個方法永遠是最佳解：
把每個絕對眾數跟其他數兩個兩個一組，
這樣每一組不論放在哪個子序列都不會影響解的正確性。

所以答案就是剩下未分組的元素，
就是 $\ell - (\ell - c) \times 2 = c \times 2 - \ell$，
沒有絕對眾數的答案就是 $1$，所以答案就是 $\max(1, c \times 2 - \ell)$。
　　  
## 套用資料結構
...
過程是相同 `counter+​+`，不同 `counter-​​​​​​-`，`cnt = 0` 就直接換數字，
這樣做並不保證是絕對眾數，但我們只要用前面的方法做一次就有答案了，
由於絕對眾數問題是無序的，所以可以透過線段樹作區間合併，
合併的規則就是相同 `counter` 相加，不同就比大小再決定怎麼相減，
這樣區間查詢就縮到 $O(\log N)$，
